...
def _resample_to_self(img, interpolation):
    ...
    img = image.resample_img(img, target_affine=np.diag(
        [vsize, vsize, vsize]), interpolation=interpolation)
    return img
    ...
    if threshold is not None:
        if threshold == 0:
            ...
            sprite = np.ma.masked_inside(sprite, -threshold, threshold,
                                         copy=False)
    # Save the sprite
    imsave(output_sprite, sprite, vmin=vmin,
           vmax=vmax, cmap=cmap, format=format)
...
def get_stat_map(stat_map_img, bg_img, cut_coords=None,
    ...
    if bg_img is not None and bg_img is not False:
        ...
        bg_img = _resample_to_self(
            bg_img, interpolation=resampling_interpolation)
        ...
        bg_img = image.new_img_like(stat_map_img, np.zeros(
            stat_map_img.shape), stat_map_img.affine)
        bg_min = 0
        bg_max = 0
        if black_bg == 'auto':
            black_bg = False
